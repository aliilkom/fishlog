...
@section('breadcrumb')
    ...
    <li class="active"><a href="{{ url('stokmasuk') }}"><i class="fa fa-sign-in"></i>Stok Masuk</a></li>
@endsection
...
@section('content')
            ...
            <table id="invoice" class="table table-bordered">
                ...
                @foreach($invoice_data as $i)
                    <tbody>
                        ...
                        <td>{{ $i->tanggal }}</td>
                        <td>
                            <a href="{{ route('exportPDF.productMasuk', [ 'id' => $i->id ]) }}" class="btn btn-xs btn-warning"><i class="glyphicon glyphicon-print"></i> Cetak</a>
                        </td>
                    </tbody>
                @endforeach
            </table>
    ...
    @include('gudang.product_masuk.form')
...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        function editForm(id) {
            ...
            $('#modal-form form')[0].reset();
            $.ajax({
                url: "{{ url('stokmasuk') }}" + '/' + id + "/edit",
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    $('#modal-form').modal('show');
                    $('.modal-title').text('Ubah Stok Masuk');
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('stokmasuk') }}";
                    else url = "{{ url('stokmasuk') . '/' }}" + id;
